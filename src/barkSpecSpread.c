...
static t_class* barkSpecSpread_class;

typedef struct _barkSpecSpread
{
    t_object x_obj;
...
/* ------------------------ barkSpecSpread -------------------------------- */
static void barkSpecSpread_resizeWindow (t_barkSpecSpread* x, t_sampIdx oldWindow, t_sampIdx window, t_sampIdx startSamp, t_sampIdx* endSamp)
{
...
static void barkSpecSpread_analyze (t_barkSpecSpread* x, t_floatarg start, t_floatarg n)
{
    t_garray* a;
    ...
    else
    {
        ...
        if (x->x_window != window)
            barkSpecSpread_resizeWindow (x, x->x_window, window, startSamp, &endSamp);
...
static void barkSpecSpread_chain_fftData (t_barkSpecSpread* x, t_symbol* s, int argc, t_atom* argv)
{
...
static void barkSpecSpread_chain_magSpec (t_barkSpecSpread* x, t_symbol* s, int argc, t_atom* argv)
{
    t_sampIdx i, windowHalf;
    t_float energySum, centroid, spread;
...
static void barkSpecSpread_chain_barkSpec (t_barkSpecSpread* x, t_symbol* s, int argc, t_atom* argv)
{
    t_filterIdx i;
    t_float energySum, centroid, spread;
...
static void barkSpecSpread_bang (t_barkSpecSpread* x)
{
    t_garray* a;
    ...
    else
    {
        ...
        barkSpecSpread_analyze (x, startSamp, window);
    }
}


static void barkSpecSpread_createFilterbank (t_barkSpecSpread* x, t_floatarg bs)
{
...
static void barkSpecSpread_spec_band_avg (t_barkSpecSpread* x, t_floatarg avg)
{
    avg = (avg < 0) ? 0 : avg;
    avg = (avg > 1) ? 1 : avg;
    x->x_specBandAvg = avg;
...
static void barkSpecSpread_filter_avg (t_barkSpecSpread* x, t_floatarg avg)
{
    avg = (avg < 0) ? 0 : avg;
    avg = (avg > 1) ? 1 : avg;
    x->x_filterAvg = avg;
...
static void barkSpecSpread_set (t_barkSpecSpread* x, t_symbol* s)
{
    t_garray* a;
...
static void barkSpecSpread_print (t_barkSpecSpread* x)
{
...
static void barkSpecSpread_samplerate (t_barkSpecSpread* x, t_floatarg sr)
{
...
static void barkSpecSpread_window (t_barkSpecSpread* x, t_floatarg w)
{
    t_sampIdx endSamp;
    ...
    barkSpecSpread_resizeWindow (x, x->x_window, w, 0, &endSamp);
}


static void barkSpecSpread_windowFunction (t_barkSpecSpread* x, t_floatarg f)
{
    f = (f < 0) ? 0 : f;
    f = (f > 4) ? 4 : f;
...
static void barkSpecSpread_powerSpectrum (t_barkSpecSpread* x, t_floatarg spec)
{
    spec = (spec < 0) ? 0 : spec;
    spec = (spec > 1) ? 1 : spec;
...
static void* barkSpecSpread_new (t_symbol* s, int argc, t_atom* argv)
{
    t_barkSpecSpread* x = (t_barkSpecSpread *)pd_new (barkSpecSpread_class);
    t_sampIdx i;
//	t_garray *a;
...
static void barkSpecSpread_free (t_barkSpecSpread* x)
{
    t_filterIdx i;
...
{
    barkSpecSpread_class =
    class_new (
        gensym ("barkSpecSpread"),
        (t_newmethod)barkSpecSpread_new,
        (t_method)barkSpecSpread_free,
        sizeof (t_barkSpecSpread),
        CLASS_DEFAULT,
        A_GIMME,
        0
    );
    ...
    class_addcreator (
        (t_newmethod)barkSpecSpread_new,
        gensym ("timbreIDLib/barkSpecSpread"),
        A_GIMME,
        0
    );

    class_addbang (barkSpecSpread_class, barkSpecSpread_bang);

    class_addmethod (
        barkSpecSpread_class,
        (t_method)barkSpecSpread_analyze,
        gensym ("analyze"),
        A_DEFFLOAT,
        A_DEFFLOAT,
        0
    ...
    class_addmethod (
        barkSpecSpread_class,
        (t_method)barkSpecSpread_chain_fftData,
    ...
    class_addmethod (
        barkSpecSpread_class,
        (t_method)barkSpecSpread_chain_magSpec,
    ...
    class_addmethod (
        barkSpecSpread_class,
        (t_method)barkSpecSpread_chain_barkSpec,
    ...
    class_addmethod (
        barkSpecSpread_class,
        (t_method)barkSpecSpread_samplerate,
        gensym ("samplerate"),
        A_DEFFLOAT,
        0
    );
    ...
    class_addmethod (
        barkSpecSpread_class,
        (t_method)barkSpecSpread_window,
        gensym ("window"),
        A_DEFFLOAT,
        0
    );
    ...
    class_addmethod (
        barkSpecSpread_class,
        (t_method)barkSpecSpread_windowFunction,
    ...
    class_addmethod (
        barkSpecSpread_class,
        (t_method)barkSpecSpread_powerSpectrum,
    ...
    class_addmethod (
        barkSpecSpread_class,
        (t_method)barkSpecSpread_createFilterbank,
        gensym ("filterbank"),
        A_DEFFLOAT,
        0
    );
    ...
    class_addmethod (
        barkSpecSpread_class,
        (t_method)barkSpecSpread_spec_band_avg,
    ...
    class_addmethod (
        barkSpecSpread_class,
        (t_method)barkSpecSpread_filter_avg,
        gensym ("filter_avg"),
        A_DEFFLOAT,
        0
    );
