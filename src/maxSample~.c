...
static t_class *maxSample_tilde_class;

typedef struct _maxSample_tilde
{
    t_object x_obj;
...
static void maxSample_tilde_bang(t_maxSample_tilde *x)
{
...
static void maxSample_tilde_window(t_maxSample_tilde *x, t_floatarg w)
{
    t_sampIdx i, window;
...
static void maxSample_tilde_overlap(t_maxSample_tilde *x, t_floatarg o)
{
...
static void maxSample_tilde_print(t_maxSample_tilde *x)
{
...
static void *maxSample_tilde_new(t_symbol *s, int argc, t_atom *argv)
{
    t_maxSample_tilde *x = (t_maxSample_tilde *)pd_new(maxSample_tilde_class);
    t_sampIdx i;
...
static t_int *maxSample_tilde_perform(t_int *w)
{
    ...
    t_maxSample_tilde *x = (t_maxSample_tilde *)(w[1]);
...
static void maxSample_tilde_free(t_maxSample_tilde *x)
{
...
{
    maxSample_tilde_class =
    class_new(
        gensym("maxSample~"),
        (t_newmethod)maxSample_tilde_new,
        (t_method)maxSample_tilde_free,
        sizeof(t_maxSample_tilde),
        CLASS_DEFAULT,
        A_GIMME,
        0
    );
    ...
    CLASS_MAINSIGNALIN(maxSample_tilde_class, t_maxSample_tilde, x_f);

    class_addbang(maxSample_tilde_class, maxSample_tilde_bang);

    class_addmethod(
        maxSample_tilde_class,
        (t_method)maxSample_tilde_print,
        gensym("print"),
        0
    );
    ...
    class_addmethod(
        maxSample_tilde_class,
        (t_method)maxSample_tilde_window,
        gensym("window"),
        A_DEFFLOAT,
        0
    );
    ...
    class_addmethod(
        maxSample_tilde_class,
        (t_method)maxSample_tilde_overlap,
        gensym("overlap"),
        A_DEFFLOAT,
        0
    );
