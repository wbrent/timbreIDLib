...
static void specCentroid_analyze(t_specCentroid *x, t_floatarg start, t_floatarg n)
{
    ...
    else
    {
        ...
        if(x->x_window != window)
            specCentroid_resizeWindow(x, x->x_window, window, startSamp, &endSamp);
...
static void specCentroid_bang(t_specCentroid *x)
{
    ...
    else
    {
        ...
        specCentroid_analyze(x, startSamp, window);
    }
}
...
//	t_garray *a;
    ...
    x->x_centroid = outlet_new(&x->x_obj, gensym("list"));
...
void specCentroid_setup(void)
{
    ...
    class_addmethod(
        specCentroid_class,
        (t_method)specCentroid_chain_fftData,
    ...
    class_addmethod(
        specCentroid_class,
        (t_method)specCentroid_chain_magSpec,
