...
static void mfcc_tilde_print(t_mfcc_tilde *x)
{
...
static void mfcc_tilde_window(t_mfcc_tilde *x, t_floatarg w)
{
...
static void mfcc_tilde_overlap(t_mfcc_tilde *x, t_floatarg o)
{
...
static void mfcc_tilde_windowFunction(t_mfcc_tilde *x, t_floatarg f)
{
    f = (f<0)?0:f;
    f = (f>4)?4:f;
...
static void mfcc_tilde_powerSpectrum(t_mfcc_tilde *x, t_floatarg spec)
{
    spec = (spec<0)?0:spec;
    spec = (spec>1)?1:spec;
...
static void *mfcc_tilde_new(t_symbol *s, int argc, t_atom *argv)
{
	...
	switch(argc)
	{
		...
		default:
			post("%s WARNING: Too many arguments supplied. Using default window size of %i and mel spacing of %f.", x->x_objSymbol->s_name, WINDOWSIZEDEFAULT, MELSPACINGDEFAULT);
			x->x_window = WINDOWSIZEDEFAULT;
			x->x_melSpacing = MELSPACINGDEFAULT;
			break;
	}
	...
	// grab memory for filterbank and filter frequencies
...
void mfcc_tilde_setup(void)
{
	...
	class_addmethod(
		mfcc_tilde_class,
        (t_method)mfcc_tilde_windowFunction,
	...
	class_addmethod(
		mfcc_tilde_class,
        (t_method)mfcc_tilde_powerSpectrum,
	...
	class_addmethod(
		mfcc_tilde_class,
        (t_method)mfcc_tilde_spec_band_avg,
	...
	class_addmethod(
		mfcc_tilde_class,
        (t_method)mfcc_tilde_filter_avg,
		gensym("filter_avg"),
		A_DEFFLOAT,
		0
	);
