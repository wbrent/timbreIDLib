...
static t_class *barkSpecSkewness_tilde_class;
...
static void barkSpecSkewness_tilde_spec_band_avg(t_barkSpecSkewness_tilde *x, t_floatarg avg)
{
    avg = (avg<0)?0:avg;
    avg = (avg>1)?1:avg;
    x->x_specBandAvg = avg;
...
static void barkSpecSkewness_tilde_filter_avg(t_barkSpecSkewness_tilde *x, t_floatarg avg)
{
    avg = (avg<0)?0:avg;
    avg = (avg>1)?1:avg;
    x->x_filterAvg = avg;
...
static void barkSpecSkewness_tilde_windowFunction(t_barkSpecSkewness_tilde *x, t_floatarg f)
{
    f = (f<0)?0:f;
    f = (f>4)?4:f;
...
static void *barkSpecSkewness_tilde_new(t_symbol *s, int argc, t_atom *argv)
{
    ...
    switch(argc)
    {
        ...
        default:
            post("%s WARNING: Too many arguments supplied. Using default window size of %i and Bark spacing of %f.", x->x_objSymbol->s_name, TID_BARKSPACINGDEFAULT, TID_WINDOWSIZEDEFAULT);
            x->x_window = TID_WINDOWSIZEDEFAULT;
            x->x_barkSpacing = TID_BARKSPACINGDEFAULT;
            break;
    }
