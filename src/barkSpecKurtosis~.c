...
static t_class *barkSpecKurtosis_tilde_class;

typedef struct _barkSpecKurtosis_tilde
{
	t_object x_obj;
...
static void barkSpecKurtosis_tilde_createFilterbank(t_barkSpecKurtosis_tilde *x, t_floatarg bs)
{
...
static void barkSpecKurtosis_tilde_spec_band_avg(t_barkSpecKurtosis_tilde *x, t_floatarg avg)
{
    avg = (avg<0)?0:avg;
    avg = (avg>1)?1:avg;
	x->x_specBandAvg = avg;
...
static void barkSpecKurtosis_tilde_filter_avg(t_barkSpecKurtosis_tilde *x, t_floatarg avg)
{
    avg = (avg<0)?0:avg;
    avg = (avg>1)?1:avg;
	x->x_filterAvg = avg;
...
static void barkSpecKurtosis_tilde_print(t_barkSpecKurtosis_tilde *x)
{
...
static void barkSpecKurtosis_tilde_window(t_barkSpecKurtosis_tilde *x, t_floatarg w)
{
...
static void barkSpecKurtosis_tilde_overlap(t_barkSpecKurtosis_tilde *x, t_floatarg o)
{
...
static void barkSpecKurtosis_tilde_windowFunction(t_barkSpecKurtosis_tilde *x, t_floatarg f)
{
    f = (f<0)?0:f;
    f = (f>4)?4:f;
...
static void barkSpecKurtosis_tilde_powerSpectrum(t_barkSpecKurtosis_tilde *x, t_floatarg power)
{
...
static void *barkSpecKurtosis_tilde_new(t_symbol *s, int argc, t_atom *argv)
{
	t_barkSpecKurtosis_tilde *x = (t_barkSpecKurtosis_tilde *)pd_new(barkSpecKurtosis_tilde_class);
	t_sampIdx i;
	...
	switch(argc)
	{
		...
		default:
			post("%s WARNING: Too many arguments supplied. Using default window size of %i and Bark spacing of %f.", x->x_objSymbol->s_name, WINDOWSIZEDEFAULT, BARKSPACINGDEFAULT);
			x->x_window = WINDOWSIZEDEFAULT;
			x->x_barkSpacing = BARKSPACINGDEFAULT;
			break;
	}
...
static t_int *barkSpecKurtosis_tilde_perform(t_int *w)
{
	t_uShortInt n;
	t_sampIdx i;

	t_barkSpecKurtosis_tilde *x = (t_barkSpecKurtosis_tilde *)(w[1]);
...
static void barkSpecKurtosis_tilde_dsp(t_barkSpecKurtosis_tilde *x, t_signal **sp)
{
	dsp_add(
		barkSpecKurtosis_tilde_perform,
		3,
		x,
		sp[0]->s_vec,
		sp[0]->s_n
...
static void barkSpecKurtosis_tilde_free(t_barkSpecKurtosis_tilde *x)
{
	t_filterIdx i;
...
{
	barkSpecKurtosis_tilde_class =
	class_new(
		gensym("barkSpecKurtosis~"),
		(t_newmethod)barkSpecKurtosis_tilde_new,
		(t_method)barkSpecKurtosis_tilde_free,
		sizeof(t_barkSpecKurtosis_tilde),
		CLASS_DEFAULT,
		A_GIMME,
		0
	);
	...
	class_addcreator(
		(t_newmethod)barkSpecKurtosis_tilde_new,
		gensym("timbreIDLib/barkSpecKurtosis~"),
		A_GIMME,
		0
	);
	
	CLASS_MAINSIGNALIN(barkSpecKurtosis_tilde_class, t_barkSpecKurtosis_tilde, x_f);

	class_addbang(barkSpecKurtosis_tilde_class, barkSpecKurtosis_tilde_bang);

	class_addmethod(
		barkSpecKurtosis_tilde_class,
		(t_method)barkSpecKurtosis_tilde_print,
		gensym("print"),
		0
	);
	...
	class_addmethod(
		barkSpecKurtosis_tilde_class,
		(t_method)barkSpecKurtosis_tilde_createFilterbank,
		gensym("filterbank"),
		A_DEFFLOAT,
		0
	);
	...
	class_addmethod(
		barkSpecKurtosis_tilde_class,
		(t_method)barkSpecKurtosis_tilde_window,
		gensym("window"),
		A_DEFFLOAT,
		0
	);
	...
	class_addmethod(
		barkSpecKurtosis_tilde_class,
        (t_method)barkSpecKurtosis_tilde_overlap,
		gensym("overlap"),
		A_DEFFLOAT,
		0
	);
	...
	class_addmethod(
		barkSpecKurtosis_tilde_class,
		(t_method)barkSpecKurtosis_tilde_windowFunction,
	...
	class_addmethod(
		barkSpecKurtosis_tilde_class,
		(t_method)barkSpecKurtosis_tilde_powerSpectrum,
	...
	class_addmethod(
		barkSpecKurtosis_tilde_class,
		(t_method)barkSpecKurtosis_tilde_dsp,
		gensym("dsp"),
		0
	);
	...
	class_addmethod(
		barkSpecKurtosis_tilde_class,
        (t_method)barkSpecKurtosis_tilde_spec_band_avg,
	...
	class_addmethod(
		barkSpecKurtosis_tilde_class,
        (t_method)barkSpecKurtosis_tilde_filter_avg,
		gensym("filter_avg"),
		A_DEFFLOAT,
		0
	);
